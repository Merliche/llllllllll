...
cur = conn.cursor()
cur.execute("""SELECT solde FROM utilisateurs """)
user_solde = cur.fetchall()
cur.execute("""SELECT last_retrait FROM utilisateurs""")
user_historique = cur.fetchall()
...
def check_if_exist(identifiants):
    ...
    cur.execute("""SELECT Name FROM utilisateurs WHERE id=?""", id,)
    NomPrenom = cur.fetchone()
    texte = "Bienvenue"+ str(NomPrenom)+ "!"
    if identifiants[1] == user_identifiants[1]:
        messagebox.showinfo(title=None, message= texte)
        return "toto"
    else:
        sys.exit()
...
def validateLogin(CB, password):
    ...
    print(identifiants)
    if check_if_exist(identifiants) == "toto":
        Window.destroy()
    else:
