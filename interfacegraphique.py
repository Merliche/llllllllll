...
def afficher_solde(identifiants):
    ...
    cur.close
    sold = Tk()
    Solde = Label(sold, text="Votre solde est :"+str(solde))
    Solde.grid()
    return solde
...
def ask_for_amount(user_solde):
    gui = Tk()
    montant=StringVar()
    l = Label( gui, text = "Entrez le montant" )
    l.pack( side = LEFT )
    e = Entry( gui, bd = 5, textvariable=montant )
    e.pack( side = RIGHT )
    validateLogin = partial(validateLogin, user_solde, montant )
    loginButton = Button(gui, text="Validez", command=validateLogin)
